          ...
          <ul class="nav nav-pills gnb">
            <c:choose>
	          	...
	          	<c:otherwise>
	            	...
	            	<li class="nav-item"><a href="../member/info?userId=${loggedMemberId}" class="nav-link">
	            	<c:choose>
	            		<c:when test="${loggedMember.realProfile eq null}">
	            			<div class=profileBox><img src="${pageContext.request.contextPath}/upload/user.png" class="profile">
	            			${loggedMemberName}
	            			</div>
	            		</c:when>
	            		
	            		<c:otherwise>
	            		<div class=profileBox><img src="${pageContext.request.contextPath}/upload/${loggedMember.realProfile}" class="profile">
	            			${loggedMemberName}
	            			</div>
	            		</c:otherwise>	
	            	</c:choose>
